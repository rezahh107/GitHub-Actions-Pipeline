"""Deterministic structural Git-history analysis with byte-preserving framing."""
...
from tools.ci_models import run_git_bytes
...
def _diagnostic_text(payload: bytes) -> str:
    """Decode failure-only Git output for display after evidence collection failed."""
    if not payload:
        return "no diagnostic output"
    return payload.decode("utf-8", errors="replace").strip() or "no diagnostic output"


def _decode_field(value: bytes, *, stream: str, index: int) -> tuple[str | None, str | None]:
    try:
        return value.decode("utf-8", errors="strict"), None
    except UnicodeDecodeError:
        return None, f"{stream} returned a non-UTF-8 field at record {index}."


def _parse_commit_records(output: bytes) -> tuple[list[dict[str, object]] | None, str | None]:
    """Parse byte-preserved NUL-delimited ``sha, subject`` records."""
    if not output:
        return [], None
    ...
    for index in range(0, len(fields), 2):
        sha_bytes, subject_bytes = fields[index], fields[index + 1]
        try:
            sha = sha_bytes.decode("ascii", errors="strict")
        except UnicodeDecodeError:
            return None, f"Git log returned a non-ASCII commit identity at record {index // 2}."
        subject, error = _decode_field(subject_bytes, stream="Git log", index=index // 2)
        if error is not None or subject is None:
            return None, error
...
def _parse_paths(output: bytes) -> tuple[list[str] | None, str | None]:
    """Parse exact Git path identities without stripping or line normalization."""
    if not output:
        return [], None
    fields = output.split(b"\0")
    if fields and fields[-1] == b"":
    ...
    for index, value in enumerate(fields):
        ...
        decoded, error = _decode_field(value, stream="Git diff-tree", index=index)
...
def _collect_commits(root: Path) -> tuple[list[dict[str, object]] | None, dict[str, object] | None]:
    ok, output = run_git_bytes(root, ["log", f"-n{COMMIT_LIMIT}", "--format=%H%x00%s", "-z"], timeout=30)
    if not ok:
        return None, _empty_history(
            "STRUCTURAL_HISTORY_FAILED",
            f"Git structural history collection failed: {_diagnostic_text(output)}",
            "Verify that Git history is readable and retry from the repository root.",
        )
    commits, parse_error = _parse_commit_records(output)
    if parse_error is not None or commits is None:
        return None, _empty_history(
            "STRUCTURAL_HISTORY_MALFORMED",
            parse_error or "Git history metadata could not be parsed.",
            "Retry with a complete Git checkout; malformed, undecodable, or truncated NUL-framed output is not used as evidence.",
        )
    for commit in commits:
        sha = str(commit["sha"])
        ok, path_output = run_git_bytes(
            root,
        ...
        if not ok:
            return None, _empty_history(
                "STRUCTURAL_HISTORY_PATHS_FAILED",
                f"Could not collect changed paths for commit {sha}: {_diagnostic_text(path_output)}",
                "Verify that the commit object and tree are readable in the local checkout.",
            )
        paths, path_error = _parse_paths(path_output)
        if path_error is not None or paths is None:
            return None, _empty_history(
                "STRUCTURAL_HISTORY_MALFORMED",
                f"Could not parse NUL-delimited paths for commit {sha}: {path_error}",
                "Retry with a complete Git checkout; malformed, undecodable, or truncated path records are not used as evidence.",
            )
        commit["paths"] = paths
    return commits, None
...
def collect_structural_history(root: Path) -> dict[str, object]:
    if not (root / ".git").exists():
        return _empty_history(
            ...
            "Run Deep Repository Upgrade from a full Git checkout.",
        )
    commits, unavailable = _collect_commits(root)
    if unavailable is not None or commits is None:
        return unavailable or _empty_history(
            "STRUCTURAL_HISTORY_UNAVAILABLE",
            "Structural history evidence was unavailable.",
            "Run from a complete readable Git checkout.",
        )
    ...
    workflow_config_churn = [
        ...
        for path, count in sorted(workflow_counts.items(), key=lambda item: (-item[1], item[0]))[:RESULT_LIMIT]
    ]
    repeated_fix_subsystems = [
        {
            "subsystem": subsystem,
        ...
        for subsystem, count in sorted(subsystem_fix_counts.items(), key=lambda item: (-item[1], item[0]))
        if count >= 2
    ][:RESULT_LIMIT]
    ...
    return {
        ...
        "evidence": evidence(
            ...
            "Structural history was derived from bounded byte-preserved NUL-delimited Git metadata and path records. Correlations do not establish causation.",
            confidence="medium",
        ),
    }
