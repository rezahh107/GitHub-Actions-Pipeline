...
This module does not emulate a shell. It recognizes only standalone,
...
control-flow bodies never become executable capability evidence. Recognized
commands running in help, listing, collection-only, dry-run, or no-run modes are
recorded as inert rather than behavioral execution evidence.
"""
...
def _contains_flag(tokens: list[str], flags: set[str]) -> bool:
    return any(token.lower() in flags for token in tokens)


def non_behavioral_reason(argv: Iterable[str]) -> str | None:
    """Return a stable reason when a recognized command cannot execute behavior.

    This is deliberately a bounded denylist for supported tools, not a claim to
    understand arbitrary command semantics.
    """
    args = list(argv)
    if not args:
        return "empty_invocation"
    executable = args[0].rsplit("/", 1)[-1].lower()
    tail = [part.lower() for part in args[1:]]

    if _contains_flag(tail, _HELP_FLAGS | _VERSION_FLAGS):
        return "help_or_version_mode"

    if executable in {"python", "python3", "py"} and tail[:2] in (["-m", "pytest"], ["-m", "unittest"]):
        tool_args = tail[2:]
        if any(flag in tool_args for flag in {"--collect-only", "--collectonly", "--co", "--fixtures", "--markers", "--trace-config", "--list-tests"}):
            return "test_collection_or_listing_mode"
    elif executable in {"pytest", "py.test", "nosetests"}:
        if any(flag in tail for flag in {"--collect-only", "--collectonly", "--co", "--fixtures", "--markers", "--trace-config", "--list-tests"}):
            return "test_collection_or_listing_mode"
    elif executable in {"npm", "pnpm", "yarn"}:
        if any(flag in tail for flag in {"--dry-run", "--if-present"}):
            return "dry_run_or_optional_mode"
    elif executable == "uv" and "--dry-run" in tail:
        return "dry_run_mode"
    elif executable == "poetry" and "--dry-run" in tail:
        return "dry_run_mode"
    elif executable == "cargo" and tail[:1] == ["test"]:
        if any(flag in tail[1:] for flag in {"--no-run", "--list"}):
            return "test_no_run_or_listing_mode"
    elif executable == "go" and tail[:1] == ["test"]:
        if any(flag == "-list" or flag.startswith("-list=") for flag in tail[1:]):
            return "test_listing_mode"
    elif executable in {"mvn", "mvnw"}:
        if any(flag in {"-dskiptests", "-dskiptests=true", "-dmaven.test.skip=true"} for flag in tail):
            return "tests_skipped_mode"
    elif executable in {"gradle", "gradlew"}:
        if any(flag in {"--dry-run", "-m"} for flag in tail):
            return "dry_run_mode"
    elif executable == "twine" and tail[:1] == ["check"] and "--strict" not in tail and len(tail) == 1:
        return "missing_artifact_operand"

    if executable in {"python", "python3", "py"} and tail[:3] == ["-m", "pip", "install"] and "--dry-run" in tail[3:]:
        return "dry_run_mode"
    return None


def classify_invocation(argv: Iterable[str]) -> list[str]:
    """Return capability families proven by one behavioral argv vector."""
    args = list(argv)
    if not args or non_behavioral_reason(args) is not None:
        return []
    executable = args[0].rsplit("/", 1)[-1].lower()
    tail = [part.lower() for part in args[1:]]
...
def parse_run_block(run: str, *, working_directory: str | None = None) -> list[dict[str, object]]:
    ...
    for line_number, raw in _logical_lines(run):
        ...
        if executable in _SHELL_BUILTINS:
            ...
            continue
        inert_reason = non_behavioral_reason(argv)
        if inert_reason is not None:
            records.append({**base, "status": "inert", "reason": inert_reason, "argv": argv, "executable": executable, "normalized": shlex.join(argv)})
            continue
        families = classify_invocation(argv)
        records.append({
            **base,
            "status": "resolved",
            "reason": "standalone_behavioral_argv",
            "argv": argv,
