...
def _records(workflows: list[dict[str, object]]) -> list[dict[str, object]]:
    return [
        ...
        for record in workflow.get("command_evidence", [])
...
def _permission_capability(workflows: list[dict[str, object]]) -> dict[str, object]:
    ...
    effective = [
        ...
        if isinstance(job, dict) and isinstance(job.get("effective_permissions"), dict)
    ]
    if not effective:
        return _cap(
            "least_privilege_workflow_permissions",
            "unknown",
            [str(workflow["path"]) for workflow in workflows],
            "Workflow permissions could not be evaluated because no valid jobs were parsed.",
            "Repair workflow job shapes and declare explicit workflow or job permissions.",
        )
    ...
    if writes:
        ...
        return _cap(
            ...
            unsafe_refs,
            "At least one job has effective write permission after applying job-over-workflow precedence.",
            "Narrow the cited effective job permission only if the job does not require write access.",
        )
    if unknown:
        ...
        return _cap(
            ...
            "unknown",
            unknown_refs,
            "One or more jobs inherit platform defaults or use malformed/unsupported permissions, so least privilege is not proven.",
            "Declare a supported explicit workflow or job permissions contract for every job.",
        )
    return _cap(
        "least_privilege_workflow_permissions",
        "operational",
        refs,
        "Every parsed job has effective read-or-none permissions after applying workflow/job precedence, including explicit empty declarations.",
    )


def build_repository_model(root: Path) -> dict[str, object]:
    root = root.resolve()
    files, truncated = iter_files(root)
    ...
    negative = sorted(path for path in tests if _negative(path_map[path]))
    ...
    declared_roots = _expand_workspace_roots(root, workspace_patterns)
    ...
    pr_test_records = [
        record
        for workflow in workflows
        if "pull_request" in workflow.get("triggers", [])
        for record in workflow.get("command_evidence", [])
        if isinstance(record, dict)
    ]
    test_ok = _family(pr_test_records, "test")
    ...
    schema_test_refs = [path for path in tests if "schema" in path.lower() or "jsonschema" in ((_text(path_map[path]) or "").lower())]
    ...
    capabilities = [
        ...
        _cap("tests_run_on_pull_requests", "operational" if test_ok and pr else "nominal" if tests and workflows else "absent", sorted({str(record.get("workflow")) for record in pr_test_records if record.get("status") == "resolved" and "test" in record.get("families", [])}) + tests, "A pull-request workflow contains a bounded, standalone, resolved test invocation." if test_ok and pr else "Tests or workflows exist, but no pull-request workflow was proven to execute a bounded recognized test invocation.", "Wire one resolved canonical test command into pull_request validation."),
        _cap("build_verified", "operational" if build_ok else "nominal" if candidates["build"] else "absent", sorted({str(record.get("workflow")) for record in resolved_records if "build" in record.get("families", [])}) + [str(item.get("basis")) for item in candidates["build"]], "A bounded recognized build invocation executes in CI." if build_ok else "A build command is declared or inferable, but CI execution was not proven." if candidates["build"] else "No build command was established.", "Execute the declared build from a clean checkout."),
        _cap("reproducible_dependency_install", "operational" if install_ok and locks else "partial" if locks or candidates["install"] else "absent", locks + sorted({str(record.get("workflow")) for record in resolved_records if "install" in record.get("families", [])}), "A lockfile and bounded deterministic install invocation were both observed in CI." if install_ok and locks else "A lockfile or deterministic install invocation is missing or disconnected.", "Use the ecosystem-native frozen install command and lockfile."),
        _cap("schema_validation", "operational" if schema_ok else "nominal" if schemas else "not_applicable", schemas + schema_test_refs + sorted({str(record.get("workflow")) for record in pr_test_records if "test" in record.get("families", [])}), "Schemas are exercised by schema-aware tests within a resolved pull-request test invocation." if schema_ok else "Schemas exist but executable compatibility validation was not proven." if schemas else "No schema surface was observed.", "Connect schemas, examples, validators, and malformed fixtures to one deterministic command." if schemas else None),
        ...
        _cap("release_artifact_validation", "operational" if release_ok else "nominal" if releases else "unknown", releases + sorted({str(record.get("workflow")) for record in resolved_records if "release" in record.get("families", [])}), "A bounded package/release verification invocation was observed." if release_ok else "Release-related paths exist, but artifact verification was not proven." if releases else "Release applicability could not be established.", "Validate package metadata and produced artifacts before release."),
    ...
    return {
        ...
        "command_evidence_boundary": {
            "version": "1.0.0",
            "resolved_record_count": len(resolved_records),
            "unsupported_record_count": len(unsupported),
            "policy": "Only standalone argv invocations recognized by the bounded parser can establish executable capabilities; shell control flow, substitutions, heredocs, and ambiguous constructs remain unsupported.",
        },
        "test_suites": {
            "files": tests,
            "negative_files": negative,
            "schema_test_files": schema_test_refs,
            "commands_observed_in_ci": sorted({str(record["normalized"]) for record in pr_test_records if record.get("status") == "resolved" and "test" in record.get("families", []) and record.get("normalized")}),
        },
        ...
        "model_evidence": evidence("derived", paths, "Repository model combines bounded file/config parsing, explicit workspace boundaries, Python AST resolution, bounded workflow command evidence, and effective GitHub permission analysis; unsupported runtime semantics remain unresolved.", confidence="high"),
    }
