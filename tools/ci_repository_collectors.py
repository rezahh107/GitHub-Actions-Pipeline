...
GITHUB_WORKFLOW_YAML_SEMANTICS_VERSION = "1.0.0"
_GITHUB_WORKFLOW_BOOL_TAG = "tag:yaml.org,2002:bool"
_GITHUB_WORKFLOW_BOOL_PATTERN = re.compile(
    r"^(?:true|True|TRUE|false|False|FALSE)$"
)


class GitHubWorkflowLoader(yaml.SafeLoader):
    """Safe loader preserving GitHub Actions YAML 1.2 boolean semantics."""
...
for resolver_key, resolvers in list(GitHubWorkflowLoader.yaml_implicit_resolvers.items()):
    GitHubWorkflowLoader.yaml_implicit_resolvers[resolver_key] = [
        (tag, pattern)
        for tag, pattern in resolvers
        if tag != _GITHUB_WORKFLOW_BOOL_TAG
    ]
GitHubWorkflowLoader.add_implicit_resolver(
    _GITHUB_WORKFLOW_BOOL_TAG,
    _GITHUB_WORKFLOW_BOOL_PATTERN,
    list("tTfF"),
)


def _construct_github_workflow_bool(
    ...
    raise ConstructorError(
        "while constructing a GitHub Actions boolean",
        node.start_mark,
        (
            f"unsupported explicit boolean scalar {value!r} under workflow YAML "
            f"semantics {GITHUB_WORKFLOW_YAML_SEMANTICS_VERSION}; use true or false"
        ),
        node.start_mark,
    )
...
GitHubWorkflowLoader.add_constructor(
    _GITHUB_WORKFLOW_BOOL_TAG,
    _construct_github_workflow_bool,
)
...
def _load_workflow(path: Path, root: Path | None = None) -> tuple[object | None, str | None]:
    """Load one workflow without YAML 1.1 boolean coercion."""
    text = _text(path, root)
    if text is None:
        return None, "unreadable_or_too_large_or_unsafe"
    try:
        return yaml.load(text, Loader=GitHubWorkflowLoader), None
...
def parse_workflow(root: Path, path: Path) -> tuple[dict[str, object], list[dict[str, object]]]:
    data, error = _load_workflow(path, root)
    rp = rel(root, path)
