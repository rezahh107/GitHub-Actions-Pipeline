...
class ExampleTests(unittest.TestCase):
    ...
    def test_gate_map_json_example_loads(self):
        data = json.loads((ROOT / "examples" / "ci_gate_map.example.json").read_text(encoding="utf-8"))
        self.assertEqual(data["gate_map_version"], "0.1")
        self.assertIn("proposed_gates", data)
        self.assertIn("rejected_gates", data)

    def test_gate_map_yaml_example_contains_required_anchors(self):
