...
class RepositoryUpgradeTests(unittest.TestCase):
    def test_modes_are_explicit_and_output_contracts_are_separate(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            minimal = build_upgrade_report(repo, mode=MINIMAL_SAFE_CI, generated_at=FIXED_TIME)
            deep = build_upgrade_report(repo, mode=DEEP_REPOSITORY_UPGRADE, generated_at=FIXED_TIME)
    ...
    def test_cold_start_keeps_structural_and_baseline_channels_active(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(report["cold_start"]["not_yet_observed_is_not_not_needed"])
            self.assertEqual(report["recommendations"]["observed_failures"], [])
    ...
    def test_repository_model_distinguishes_nominal_from_operational(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            nominal.mkdir(); operational.mkdir()
            materialize(nominal, "cold-start-python-cli")
            materialize(operational, "operational-python-package")
            nominal_states = {item["capability_id"]: item["state"] for item in build_repository_model(nominal)["capabilities"]}
            operational_states = {item["capability_id"]: item["state"] for item in build_repository_model(operational)["capabilities"]}
            self.assertNotEqual(nominal_states["tests_run_on_pull_requests"], "operational")
            self.assertEqual(operational_states["tests_run_on_pull_requests"], "operational")
            self.assertEqual(operational_states["build_verified"], "operational")
            self.assertEqual(operational_states["reproducible_dependency_install"], "operational")
    ...
    def test_composable_profiles_match_multiple_repository_characteristics(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            composition = compose_profile_contributions(detect_profiles(model, catalog), catalog)
            self.assertIn("python-library-package", composition["selected_profiles"])
            self.assertIn("python-cli", composition["selected_profiles"])
            self.assertIn("tests_run_on_pull_requests", composition["expected_capabilities"])
    ...
    def test_contract_repository_gets_schema_invariant_without_history(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            report = build_upgrade_report(repo, mode=DEEP_REPOSITORY_UPGRADE, generated_at=FIXED_TIME)
            ids = {item["recommendation_id"] for item in report["recommendations"]["structural_invariants"]}
    ...
    def test_ranking_is_bounded_deterministic_and_rejects_missing_factors(self):
        factors = {"risk_reduction":3,"invariant_criticality":3,"regression_detection":2,"silent_failure_exposure":2,"evidence_strength":3,"maintainability":2,"reversibility":3,"implementation_complexity":1,"execution_time":1,"noise_risk":1,"maintenance_cost":1,"control_overlap":0}
    ...
    def test_fake_github_telemetry_detects_recurring_failures(self):
        def transport(url, headers):
            ...
            return {"workflow_runs":[{"id":2,"name":"Validate","event":"pull_request","status":"completed","conclusion":"failure","head_sha":"a"*40,"head_branch":"feature","created_at":"2026-07-10T00:00:00Z","updated_at":"2026-07-10T00:02:00Z"},{"id":1,"name":"Validate","event":"pull_request","status":"completed","conclusion":"failure","head_sha":"b"*40,"head_branch":"feature","created_at":"2026-07-09T00:00:00Z","updated_at":"2026-07-09T00:01:00Z"}]}
        telemetry = collect_github_telemetry("owner/repo", token="test-token", transport=transport)
        self.assertEqual(telemetry["status"], "collected")
        self.assertEqual(telemetry["recurring_failures"], [{"workflow":"Validate","failure_count":2}])
    ...
    def test_reports_are_deterministic_with_fixed_timestamp(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            repo = Path(tmpdir)
            materialize(repo, "cold-start-python-cli")
            first = build_upgrade_report(repo, mode=DEEP_REPOSITORY_UPGRADE, generated_at=FIXED_TIME)
            second = build_upgrade_report(repo, mode=DEEP_REPOSITORY_UPGRADE, generated_at=FIXED_TIME)
            self.assertEqual(first, second)
            self.assertEqual(first["evidence_sha256"], compute_upgrade_sha256(first))
    ...
    def test_invalid_mode_is_rejected_with_stable_code(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            with self.assertRaises(UpgradeContractError) as raised:
                build_upgrade_report(Path(tmpdir), mode="unknown", generated_at=FIXED_TIME)
    ...
    def test_operational_capability_does_not_generate_duplicate_gap(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            report = build_upgrade_report(repo, mode=DEEP_REPOSITORY_UPGRADE, generated_at=FIXED_TIME)
            ids = {item["recommendation_id"] for item in report["recommendations"]["ranked"] if item["affected_capability"] == "tests_run_on_pull_requests"}
            self.assertNotIn("INV-TESTS-EXECUTED-ON-PR", ids)
            self.assertNotIn("BASE-TESTS-RUN-ON-PULL-REQUESTS", ids)
